...
notify_user() {
	if [[ "$(get_volume)" == " " ]]; then
		notify-send -a -r -h string:x-canonical-private-synchronous:volume -i "$(get_icon)" "Volume: Muted"
	else
		notify-send -a -r -h string:x-canonical-private-synchronous:volume -i "$(get_icon)" "Volume: $(get_volume)"
	fi
}
